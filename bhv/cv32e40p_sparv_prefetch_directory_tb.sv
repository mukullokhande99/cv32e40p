...
module cv32e40p_sparv_prefetch_directory_tb;
  ...
  task expect(input logic cond, input string msg); begin
  ...
  initial begin
    ...
    expect(probe_hit_o, "prefetched line must probe hit");

    // First architectural demand of the line marks the prefetch useful.
    demand_lookup_i=1; demand_hit_i=1; demand_pc_i=32'h104; #1;
    expect(useful_prefetch_o, "first demand hit must mark prefetch useful");
    tick(); demand_lookup_i=0; demand_hit_i=0; #1;
    expect(!useful_prefetch_o, "useful event must pulse once");
    ...
    expect(wasted_prefetch_o, "replacement of unused prefetched line must be waste");
    ...
    expect(!probe_hit_o, "invalidate must clear directory");
